...
class GrayVisualizer(BaseVisualizer):
    def __init__(self,
    ...
    def visualize(self,
       ...
       """
        ...
        # 绘制真实值
        for t in range(T):
            axes[0, t].imshow(true[t], cmap='gray', vmin=0, vmax=1)
            axes[0, t].set_title(f'True t={t}')
            axes[0, t].axis('off')

        # 绘制预测值
        for t in range(T):
            axes[1, t].imshow(pred[t], cmap='gray', vmin=0, vmax=1)
            axes[1, t].set_title(f'Pred t={t}')
            axes[1, t].axis('off')

        # 绘制误差图
        for t in range(T):
            axes[2, t].imshow(err[t], cmap='gray', vmin=0, vmax=1)
